...
def parse_factor(tree: Node, lexemes: List[str]) -> bool:
    ...
    if parse_identifier(new_node, lexemes) \
        ...
        and parse_aryth_expr(new_node, lexemes) \
...
def parse_aryth_expr(tree: Node, lexemes: List[str]) -> bool:
    new_node = Node("\<арифметическое выражение\>")
    if parse_therm(new_node, lexemes) \
        and parse_aryth_expr_stroke(new_node, lexemes):
...
def parse_expr(tree: Node, lexemes: List[str]) -> bool:
    ...
    if parse_aryth_expr(new_node, lexemes):
...
def parse_operator(tree: Node, lexemes: List[str]) -> bool:
    ...
    if parse_identifier(new_node, lexemes) and \
        ...
        parse_expr(new_node, lexemes) and \
        parse_lex(new_node, lexemes, ';'):
...
def tokenize(string: str) -> List[str]:
    ...
    while curr_pos < len(string):
        if string[curr_pos: min(curr_pos + 5, len(string))] in ['begin']:
            ...
            curr_pos += 5
        elif string[curr_pos : min(curr_pos + 3, len(string))] in ['end']:
            tokens += [string[curr_pos: min(curr_pos + 3, len(string))]]
            curr_pos += 3
        elif string[curr_pos: min(curr_pos + 2, len(string))] in ['<>', '<=', '>=', ':=']:
